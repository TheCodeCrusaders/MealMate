...
function createItem(element, index) {
  ...
  dateAdded.textContent = element.dateAdded;

  let currentDate = new Date(); //Uses current date by default
...
function sortSelected(sortSelection) {
  ...
  if (sortSelection === "Recently added") {
    ...
    rows.sort((a, b) => {
      const dateA = new Date(a.getAttribute("data-"));
      const dateB = new Date(b.getAttribute("data-"));
      console.log("DateA:\n", dateA + "\n");
      console.log("DateB:\n", dateB) + "\n";
      return new Date();
    });
